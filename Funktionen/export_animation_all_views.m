%% Funktion zum Exportieren der Animation einer Eigenform (Alle Ansichten)
...
function export_animation_all_views(path, phi, knotenkoord, balkenkoord, ...
    dof_data, surface_matrix, scale, frame_rate, direction, freq, oma)

    % Erstellen eines Bildfensters, welches geupdated wird
    figure('units','normalized','outerposition',[0 0 1 1]);

    % Wenn die OMA-Methode eingegeben wurde, Pfad mit OMA-Methode
    % konstruieren
    if nargin == 11
        file_name = strcat('Eigenform_', num2str(freq), '_Hz_Alle_', oma, '.avi');

    % Sonst Pfad ohne OMA-Methode konstruieren
    else
        file_name = strcat('Eigenform_', num2str(freq), '_Hz_Alle.avi');
    end
    ...
    ansicht = [-37.5, 30;   % 3D-Ansicht
               0,  0;       % Seitenansicht
             -90,  0;       % Querschnittansicht
               0, 90];      % Draufsicht
                    
    % Titel für verschiedene Ansichten
    strings = {'\textbf{3D-Ansicht}','\textbf{Seitenansicht}','\textbf{Querschnittsansicht}','\textbf{Draufsicht}'};
    ...
    for j=1:size(phi,2)
        ...
        if nargin == 11
            main_head_line = ['\textbf{Eigenform f\"ur Frequenz ', num2str(freq), ' Hz anhand ', char(oma), '}'];
    
        % Sonst Titel ohne OMA-Methode anzeigen
        else
            main_head_line = ['\textbf{Eigenform f\"ur Frequenz ', num2str(freq), ' Hz}'];
        end         

        % Titel für alle Subplots
        sgtitle(main_head_line, 'FontSize', 15);
