%% Funktion zum Exportieren der maximalen Auslenkung einer Eigenform (Alle Ansichten)
...
function export_displacement_all_views(path, phi, knotenkoord, balkenkoord, ...
    dof_data, surface_matrix, scale, direction, freq, oma)
    ...
    head_line = {'\textbf{3D-Ansicht}','\textbf{Seitenansicht}','\textbf{Querschnittsansicht}','\textbf{Draufsicht}'};
    ...
    for j = 1:4
        ...
        % Ansicht wählen
        angle_selected = angle(j, :);
        view(graph, angle_selected);
        hold(graph, 'on');

        % Titel
        title(graph, head_line{j})
    ...
    % Wenn die OMA-Methode eingegeben wurde, muss sie in den Titel und 
    % Pfad angezeigt werden
    if nargin == 10
        main_head_line = ['\textbf{Eigenform f\"ur Frequenz ', num2str(freq), ' Hz anhand ', char(oma), '}'];
        file_name = strcat(path, '\Eigenform_', string(freq), '_Hz_Alle_', oma, '.pdf');

    % Sonst Titel und Pfad ohne OMA-Methode anzeigen
    else
        main_head_line = ['\textbf{Eigenform f\"ur Frequenz ', num2str(freq), ' Hz}'];
        file_name = strcat(path, '\Eigenform_', string(freq), '_Hz_Alle.pdf');
    end

    % Titel erzeugen
    sgtitle(main_head_line, 'FontSize', 10);
