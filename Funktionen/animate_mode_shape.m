%% Funktion zur Animation der Eigenform
...
%                       src = Button dieser Funktion, wo der Timer integriert wurde
%                       phi = Eigenform
...
function animate_mode_shape(graph, src, phi, knotenkoord, balkenkoord, dof_data, ...
    surface_matrix, scale, ansicht, direction, freq, oma)

    % Graph zurücksetzen
    cla(graph);

    % Aktuellen Zeitschritt des Timers holen (die j-te Skalierungsstufe
    % wird später geholt)
    j = src.UserData.time_step;
    ...
    for i = 1:size(surface_matrix, 1)
        ...
        % Koordinaten der Knoten der Linie holen
    ...
    % Wenn alle Bilder durchgelaufen sind, dann wieder von vorne anfangen
    if j == size(phi,2)

        % Zeitschritt zurücksetzen
        j = 0;
        src.UserData.time_step = j;
    end

    % Zeitschritt um 1 erhöhen
    src.UserData.time_step = j+1;
