%% Callback für Button zum Exportieren der Geometrie (Modus "Zusammenführung")
% Author: Qiao Dawn Puah
% Datum letzter Bearbeitung: 30.06.2026

function export_geometry_button_pushed_merge(app)
    
    % Nötige Variablen holen
    path = app.geometry_path_edit_field_merge.Value;        % Pfad zum Zielordner
    assign_tick = app.assign_check_merge.Value;             % Wahl für Zuweisungen
    node_tick = app.node_check_merge.Value;                 % Wahl für Knoten
    line_tick = app.line_check_merge.Value;                 % Wahl für Linien
    surface_tick = app.surface_check_merge.Value;           % Wahl für Flächen
    ...
    try
        ...
        if assign_tick
            ...
            % Datei erstellen
            file_name = strcat(path, '\Zuweisungen_zusammen.txt');
            fid = fopen(file_name, 'w+');
            fprintf(fid, 'Gruppe          Sensor          Knoten\n');
        ...
        if node_tick
            ...
            if ~isempty(node_matrix)
                ...
                % Datei erstellen
                file_name = strcat(path, '\Knoten_zusammen.txt');
                fid = fopen(file_name, 'w+');
        ...
        if line_tick
            ...
            if ~isempty(line_matrix)
                ...
                % Datei erstellen
                file_name = strcat(path, '\Linien_zusammen.txt');
                fid = fopen(file_name, 'w+');
        ...
        if surface_tick
            ...
            if ~isempty(surface_matrix)
                ...
                % Datei erstellen
                file_name = strcat(path, '\Flaechen_zusammen.txt');
                fid = fopen(file_name, 'w+');
