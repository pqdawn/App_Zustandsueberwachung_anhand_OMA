%% Callback für Button zum Exportieren der Eigenform (Modus "Modalanalyse")
% Author: Qiao Dawn Puah
% Datum letzter Bearbeitung: 30.06.2026

function export_eigenform_button_pushed(app)
    ...
    freq_damp_table = app.freq_damp_table3.Data;                            % Tabelle für Ergebnisse
    path = app.eigenform_path_edit_field.Value;                             % Pfad zum Zielordner
    displacement_tick = app.displacement_check.Value;                       % Checkbox für maximale Auslenkung
    animation_tick = app.animation_check.Value;                             % Checkbox für Animation
    x_tick = app.x_direct_check3.Value;                                     % Checkbox für x-Richtung
    y_tick = app.y_direct_check3.Value;                                     % Checkbox für y-Richtung
    z_tick = app.z_direct_check3.Value;                                     % Checkbox für z-Richtung
    scale = app.scale_spinner2.Value;                                       % Skalierungsfaktor für die Eigenform
    speed = app.speed_dropdown2.Value;                                      % Wiedergabegeschwindigkeit
    view_all_tick = app.view_all_check.Value;                               % Checkbox für alle Ansichten
    view_3d_tick = app.view_3D_check.Value;                                 % Checkbox für 3D-Ansicht
    view_side_tick = app.view_side_check.Value;                             % Checkbox für Seitenansicht
    view_cross_tick = app.view_cross_check.Value;                           % Checkbox für Querschnittsansicht
    view_plan_tick = app.view_plan_check.Value;                             % Checkbox für Draufsicht
    ...
    try
        ...
        % Matrix für Eigenformen aller OMA-Methoden instanziieren
        eigenform_all_oma = [];

        % Eigenformen verschiedener Methoden zusammenfügen
        for i = 1:length(oma_uniq)

            switch oma_uniq(i)
                case "FDD"
                    eigenform_all_oma = [eigenform_all_oma, [result_fdd.mode_shape]];

                case "EFDD"
                    eigenform_all_oma = [eigenform_all_oma, [result_efdd.mode_shape]];                    

                case "SSI-COV"
                    eigenform_all_oma = [eigenform_all_oma, [result_ssi_cov.mode_shape_mean]];

                case "SSI-DATA"
                    eigenform_all_oma = [eigenform_all_oma, [result_ssi_data.mode_shape_mean]];                    
            end
        end

        % Fehlermeldung falls die Anzahl der Eigenformen nicht mit der 
        % Anzahl der Eigenfrequenzen übereinstimmt
        if (size(eigenform_all_oma,2) ~= size(freq_matrix,1))
            error('Anzahl der Eigenformen stimmt nicht mit der Anzahl der Eigenfrequenzen überein!')
        end

        % Nur die gewählten OMA-Methoden, Eigenfrequenzen und
        % Eigenformen weiter betrachten
        oma_matrix = oma_matrix(to_export, :);
        freq_matrix = freq_matrix(to_export, :);
        eigenform_all_oma = eigenform_all_oma(:, to_export);
        ...
        for i = 1:size(freq_matrix,1)
            ...
            % Aktuelle OMA-Methode
            oma_selected = oma_matrix(i);

            % Aktuelle Eigenfrequenz
            eigenfreq_selected = freq_matrix(i);

            % Aktuelle Eigenform
            eigenform_selected = eigenform_all_oma(:, i);

            % Falls gemessene Richtungen weniger als 3, müssen die Eigenformen
            % angepasst werden, damit die Größe der Matrizen geeignet für die
            % Funktionen der Darstellung und Animation ist
            ...
            if displacement_tick
                ...
                if view_all_tick
                    ...
                    export_displacement_all_views(path, eigenform_selected, node_matrix, line_matrix, assign_matrix, surface_matrix, scale, direction, eigenfreq_selected, oma_selected);
                end
                ...
                if view_3d_tick
                    ...
                    % Pfad konstruieren
                    file_name = strcat(path, '\Eigenform_', string(freq_matrix(i,1)), '_Hz_3D_', oma_selected, '.pdf');

                    % Plotten
                    fig = figure;
                    ax = axes;
                    plot_eigenform(ax, eigenform_selected, node_matrix, line_matrix, assign_matrix, surface_matrix, scale, view, direction, eigenfreq_selected, oma_selected);
                    xlabel(ax, '\textit{x}');
                    ylabel(ax, '\textit{y}');
                    zlabel(ax, '\textit{z}');
                ...
                if view_side_tick
                    ...
                    % Pfad konstruieren
                    file_name = strcat(path, '\Eigenform_', string(freq_matrix(i,1)), '_Hz_Seite_', oma_selected, '.pdf');
                ...
                if view_cross_tick
                    ...
                    % Pfad konstruieren
                    file_name = strcat(path, '\Eigenform_', string(freq_matrix(i,1)), '_Hz_Querschnitt_', oma_selected, '.pdf');
                ...
                if view_plan_tick
                    ...
                    % Pfad konstruieren
                    file_name = strcat(path, '\Eigenform_', string(freq_matrix(i,1)), '_Hz_Drauf_', oma_selected, '.pdf');
            ...
            if animation_tick
                ...
                export_animation_diff_views(path, phi_matrix, node_matrix, line_matrix, assign_matrix, surface_matrix, scale, frame_rate, view, direction, eigenfreq_selected, oma_selected);

                % Alle Ansichten gewählt
                if view_all_tick

                    % Animieren und und als AVI-Datei exportieren
                    export_animation_all_views(path, phi_matrix, node_matrix, line_matrix, assign_matrix, surface_matrix, scale, frame_rate, direction, eigenfreq_selected, oma_selected);
                end
            end
        end
