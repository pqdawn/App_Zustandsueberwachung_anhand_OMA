%% Callback für Button zum Durchführen der Modalanalyse (SSI-DATA)
% Author: Qiao Dawn Puah
% Datum letzter Bearbeitung: 30.06.2026

function run_analysis_button_pushed_ssi_data(fig, app)

    % Nötige Variablen holen
    freq_limit = str2double(app.freq_limit_edit_field.Value);           % Frequenzgrenze
    num_row_hankel = str2double(app.num_row_hankel_edit_field.Value);   % Zeilenanzahl der Hankelmatrix
    num_col_hankel = str2double(app.num_col_hankel_edit_field.Value);   % Spaltenanzahl der Hankelmatrix
    max_model_order = str2double(app.max_model_order_edit_field2.Value);% Maximale Model-Order
    freq_variation = app.freq_variation_spinner2.Value;                 % Grenzwert für stabile Frequenz
    damping_variation = app.damping_variation_spinner2.Value;           % Grenzwert für stabilen Dämpfungsgrad
    mac = app.mac_spinner5.Value;                                       % Grenzwert für "MAC-Bedingung erfüllt"
    auto_pol_choice = app.auto_pol_switch2.Value;                       % Wahl für automatische Polauswahl
    crit_stable_freq_tick = app.criteria_stable_freq_check2.Value;      % Wahl für stabile Frequenz
    crit_stable_damp_tick = app.criteria_stable_damp_check2.Value;      % Wahl für stabilen Dämpfungsgrad
    crit_mac_tick = app.criteria_mac_check2.Value;                      % Wahl für MAC-Bedingung erfüllt
    num_mode_known_choice= app.num_mode_known_ssi_data.Value;           % Wahl für Anzahl der Moden
    cluster_distance = app.cluster_distance_spinner2.Value;             % Cluster-Distanzschwelle
    min_cluster_size = app.min_cluster_size_spinner2.Value;             % Mindestgröße eines Clusters
    target_num = app.target_num_spinner_ssi_data.Value;                 % Zielanzahl
    ...
    % übereinstimmen, SSI-DATA durchführen
    if ~isequal(app.fig.UserData.cache.modal.used_parameter.data, app.fig.UserData.cache.modal.current_parameter.data)

        % Status aktualisieren
        update_status(status, lamp, '>> SSI-DATA fängt gleich an...', 'warnung');
        pause(1);  
        ...
        all_pole = compute_ssi_data(fig, sensor_data, ...
         sampling_freq, freq_limit, num_row_hankel, num_col_hankel, max_model_order, freq_variation, damping_variation, mac);
        ...
        plot_stab_diag(stab_graph, all_pole, all_poles_tick, ...
            ...
            filter_or_tick, filter_and_tick, 'SSI-DATA', 1, freq_limit, slider, ...
            range_check, range_edit_field);

        % Falls abgeleitete Messdaten vorhanden sind, muss SSI-DATA auch darauf
        % durchgeführt werden
        if ~isnan(data_matrix_diff)
        
            % Status aktualisieren
            update_status(status, lamp, '>> SSI-DATA wird noch auf abgeleitete Messdaten durchgeführt...', 'warnung');
            pause(1);
            ...
            % SSI-DATA durchführen
            all_pole = compute_ssi_data(fig, sensor_data_diff, ...
                sampling_freq, freq_limit, num_row_hankel, num_col_hankel, max_model_order, freq_variation, damping_variation, mac);

            % Variable speichern
            app.fig.UserData.cache.modal.data_all_pole_diff = all_pole;   
            ...
            plot_stab_diag(stab_graph, all_pole, all_poles_tick, ...
                stable_freq_tick, stable_damp_tick, mac_tick, pole_chosen_tick, ...
                filter_or_tick, filter_and_tick, 'SSI-DATA', 2);    
        end

        % Parameter für SSI-DATA speichern
        app.fig.UserData.cache.modal.used_parameter.data = app.fig.UserData.cache.modal.current_parameter.data; 

        % Nachricht für Status
        message = '>> SSI-DATA durchgeführt und Stabilisationsdiagramm aktualisiert';        
    ...
    % Sonst
        ...
        % Nachricht für Status
        message = '>> Vorherige Ergebnisse der SSI-DATA geholt und Stabilisationsdiagramm aktualisiert';  
    end  
    ...
    if strcmp(auto_pol_choice, 'An')
        ...
        if ~isequal(app.fig.UserData.cache.modal.used_parameter.data_cluster, app.fig.UserData.cache.modal.current_parameter.data_cluster)...
        ...
        % Sonst
            ...
            unselected_counter = app.fig.UserData.cache.modal.data_unselected_counter;
        ...
        if ~isnan(data_matrix_diff)
            ...
            % Stabilisationsdiagramm plotten
            plot_stab_diag(stab_graph, all_pole, all_poles_tick, ...
                stable_freq_tick, stable_damp_tick, mac_tick, pole_chosen_tick, filter_or_tick, filter_and_tick, 'SSI-DATA', 2);                

            % Variablen für gewählte Pole speichern
            app.fig.UserData.cache.modal.data_all_pole_diff = all_pole;
            app.fig.UserData.cache.modal.data_selected_pole_diff = selected_pole;
            app.fig.UserData.cache.modal.data_unselected_counter = unselected_counter;
        ...
        % aktualisiert werden
            ...
            % Stabilisationsdiagramm plotten
            plot_stab_diag(stab_graph, all_pole, all_poles_tick, ...
                stable_freq_tick, stable_damp_tick, mac_tick, pole_chosen_tick, filter_or_tick, filter_and_tick, 'SSI-DATA', 1);                 
        ...
        app.fig.UserData.cache.modal.used_parameter.data_cluster = app.fig.UserData.cache.modal.current_parameter.data_cluster;    
    ...
    % Falls automatische Polauswahl ausgeschaltet ist
        ...
        % Vorherige Ergebnisse, die von SSI-DATA abhängig sind, löschen
        app.fig.UserData.cache.modal.data_selected_pole = struct([]);
        app.fig.UserData.cache.modal.data_selected_pole_diff = struct([]);
        app.fig.UserData.cache.modal.data_unselected_counter = struct([]);        
    end        
end